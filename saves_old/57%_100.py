...
ISOLATED2 = [20, 28, 35, 38]
SWAPS = [
    # 20
    (28, 31), (31, 29), # 28
    (35, 37), (37, 32), (32, 33), # 35
    (38, 41), (38, 39), (41, 40), (39, 40), (40, 39), # 38
]
...
def handle_claim_territory(query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    if not my_t and 38 in unclaimed_t:
        return game.move_claim_territory(query, 38)
    ...
    # my_t = game.state.get_territories_owned_by(m_id())
    # ISOLATED2 = [t for t in ISOLATED2 if t in my_t]

    # for t1, t2 in SWAPS:
    #     if t1 in ISOLATED2 and t2 in unclaimed_t:
    #         ISOLATED2.remove(t1)
    #         ISOLATED2.append(t2)
    #         return game.move_claim_territory(query, t2)
    
    # t = min(unclaimed_t, key=lambda x:(len(f_adj(x)), -len(f_me(x))))
...
def handle_place_initial_troop(query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    # t = min(ISOLATED2, key=lambda x: (len(f_adj(x)), -len(f_me(x))))
    t = min(border_t, key=T)
...
def handle_distribute_troops(query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    # distribute evenly
    num_alive = sum(p.alive for p in game.state.players.values())
    if len(must_place) != 0:
...
def handle_fortify(query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    my_t = game.state.get_territories_owned_by(m_id())
    border_t = game.state.get_all_border_territories(my_t)
    source = max(my_t, key=T)
    if source not in border_t:
        path = find_shortest_path_from_vertex_to_set(source, border_t)
