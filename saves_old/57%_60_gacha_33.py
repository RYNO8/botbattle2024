...
def handle_place_initial_troop(query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    t = max(border_t, key=lambda x: len(f_opp(x)))
...
def getBestPath(my_t: list[int], border_t: list[int], total_troops: int) -> list[int]:
    ...
    for _ in range(100):
        ...
        for x in path[1:]: freq[O(x)] += 1
        numDefeated = sum(freq[p] == len(game.state.get_territories_owned_by(p)) for p in game.state.players.keys())
        cntDefeated = sum(p.card_count for p_id, p in game.state.players.items() if freq[p_id] == len(game.state.get_territories_owned_by(p_id)))
        ...
        borderSz = len(game.state.get_all_border_territories(my_t + path[1:]))
        currScore = (cntDefeated, numContinents, len(path) - borderSz)
...
def handle_distribute_troops(query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    if len(must_place) != 0:
        ...
        distributions[t] += 2
        total_troops -= 2
    if total_troops + max(border_t, key=lambda x: T(x) + distributions[x])>= 7:
        # -5 gets 46%_60
        # -6 gets 55%_60
        # -7 gets 48%_60
        bot_state.targets = getBestPath(my_t, border_t, max(total_troops - 7, 0))
        bot_state.border = game.state.get_all_border_territories(my_t + bot_state.targets)
        print("=>", len(game.state.recording), bot_state.targets, bot_state.border)
