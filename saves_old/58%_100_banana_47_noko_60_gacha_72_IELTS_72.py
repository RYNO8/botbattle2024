...
SPREE_THRESH, LEAVE_T, SCORE_D, SCORE_C, CARD_THRESH = 7, 3, 1, 0, 6
# SPREE_THRESH, LEAVE_T, SCORE_D, SCORE_C = map(int, __file__.split("_")[-5:-1])
UNCLAIMED_PRIORITY = [35]

# for filename in w_{5,6,7,8}_{2,3,4}_{0,1}_{0,1}_.py; do cp w.py "${filename}"; done

# round 2: banana_74_noko_63_gacha_50_IELTS_72
# 76  Gacha Funds               50.0%_20 [10, 6, 0, 3, 1]
# 80  ryno                      49.0%_100 [49, 7, 14, 12, 18]
# 90  しかのこのこのここしたんたん            37.5%_16 [6, 5, 3, 1, 1]
# 56  Winning team              35.7%_14 [5, 5, 3, 0, 1]
# 113 BOB                       28.6%_14 [4, 5, 3, 1, 1]
# 75  IELTS5.5                  28.6%_14 [4, 5, 3, 2, 0]
# 89  Banana                    26.7%_15 [4, 3, 2, 5, 1]
# 66  Team Name                 16.7%_18 [3, 8, 2, 2, 3]
# 61  Existential Birds         15.4%_13 [2, 1, 4, 3, 3]
# 88  Alpha Team                15.0%_20 [3, 5, 4, 6, 2]
# 87  Burt Picklejuice          14.3%_14 [2, 3, 3, 3, 3]
# 93  The Bots                  14.3%_14 [2, 1, 6, 4, 1]
# 112 Team 1                    12.5%_16 [2, 3, 5, 4, 2]
# 105 pikachu                   8.3%_12 [1, 2, 3, 3, 3]
# 96  Apex                      7.7%_13 [1, 4, 1, 5, 2]
# 63  MIB                       6.7%_15 [1, 3, 4, 3, 4]
# 68  Butter Paneer             5.9%_17 [1, 3, 4, 2, 7]
...
class BotState():
    ...
    def getBestPath(self, my_t: list[int], border_t: list[int], total_troops: int):
        ...
        print("=>", len(game.state.recording), self.bestScore, self.targets, self.border)
        if DEBUG:
            profiler.disable()
...
def handle_place_initial_troop(query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    my_t = game.state.get_territories_owned_by(m_id())
    base_t = containing_base(my_t, 35) or my_t
    border_t = game.state.get_all_border_territories(base_t)
    bot_state.getBestPath(my_t, border_t, 25 - sum(map(T, my_t)))
    t = bot_state.targets[0] if bot_state.targets else min(border_t, key=lambda x: dist(x, 2))
...
def handle_distribute_troops(query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    if len(must_place) != 0:
        ...
        distributions[t] += 2
        total_troops -= 2
    if total_troops + max(border_t, key=lambda x: T(x) + distributions[x])>= SPREE_THRESH:
...
def handle_troops_after_attack(query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    if move_attack.attacking_territory in bot_state.border:
        return game.move_troops_after_attack(query, max(min_move_t, T(attacking_t) - 3))
    else:
