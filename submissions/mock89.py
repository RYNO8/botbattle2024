...
def getBestPath(my_t: list[int], border_t: list[int], total_troops: int) -> list[int]:
    ...
    bestScore = (-1e9, -1e9)
    bestPath = []
    # at 100 pretty safe with TLE
    # at 400, win rate from 46% -> 50% over 60 (maybe just noise?)
    for _ in range(100):
        ...
        numDefeated = sum(freq[p] == len(game.state.get_territories_owned_by(p)) for p in freq)
        # defeatedCardCnt = sum(p.card_count for p_id, p in game.state.players.items() if freq[p_id] == len(game.state.get_territories_owned_by(p_id)))

        # NOTE: dont try to minimise or maximise touching
        # touching = set() # owo?
        # for x1 in path[1:]: touching |= set(O(f_adj(x1)))
        numContinents = sum(all(c in seen for c in C) for C in game.state.map._continents.values())

        # maximise defeatedCardCnt, then maximise path length
        currScore = (numDefeated, numContinents + len(path))
...
def handle_distribute_troops(query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    if len(must_place) != 0:
        ...
        total_troops -= 2
    if total_troops >= 12:
        bot_state.targets = getBestPath(my_t, border_t, total_troops - 4)
...
def handle_attack(query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    # pls dont get mad at me, im trying to see the logs of games exceeding the maximum recording
    # i will make sure im not spamming to gain leaderbaord winrate
