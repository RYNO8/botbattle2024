...
def handle_place_initial_troop(query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    t = min(border_t, key=lambda x: T(x) - min(T(f_opp(x))))
...
def getBestPath(my_t: list[int], border_t: list[int], total_troops: int) -> list[int]:
    # the border size should increase by no more than 2
    ...
    for _ in range(100):
        ...
        seen = set(my_t)
        while True:
            opp_x = [x for x in f_adj(path[-1]) if x not in seen]
            if not opp_x: break
            x = random.choice(opp_x)
            path.append(x)
            seen.add(x)
            
            if getReqT(path) > total_troops: break
            borderSz = len(game.state.get_all_border_territories(seen))
...
def handle_distribute_troops(query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    remaining_t = total_troops - 2 * (len(must_place) != 0)
    bestPath = getBestPath(my_t, border_t, remaining_t)
    print(len(game.state.recording), bestPath)
    reqT = getReqT(bestPath)
    assert 0 <= reqT <= remaining_t
    bot_state.attacks = [(i, j) for i, j in zip(bestPath, bestPath[1:])]
    ...
    if len(must_place) != 0:
        t = min(must_place, key=lambda x: ([100, -100][x == bestPath[0]], potential(x)))
        assert total_troops >= 2
        distributions[t] += 2
        total_troops -= 2
    distributions[bestPath[0]] += reqT
    total_troops -= reqT
...
def handle_attack(query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    for (t, x) in bot_state.attacks:
        if t in my_t and T(t) >= 3 and x in f_opp(t):
