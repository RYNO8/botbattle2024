...
def handle_claim_territory(query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    global V
    if V == []:
        if all(O(t)==None for t in [40, 39, 38, 41]):
            V = [40, 39, 38, 41, 24, 18, 17, 22, 19, 23, 25]
        elif all(O(t)==None for t in [29, 30, 31, 28]):
            V = [29, 30, 31, 28, 2, 3, 8, 7]
        else:
            V = [20] # NOTE: ryno added this
    ...
    t = min(unclaimed_territories, key=lambda x: len(f_me(x)))
    return game.move_claim_territory(query, t)
...
def handle_place_initial_troop(query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    random.shuffle(border_t) # NOTE: ryno added this
...
def handle_attack(query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    # for x in candidates:
    #     if T(x) >= ATTACK_THRESH: continue
    #     adj = sorted(f_me(x), key=T, reverse=True)
    #     for t in adj:
    #         if T(t) >= 6 and T(t) >= 4 + T(x):
