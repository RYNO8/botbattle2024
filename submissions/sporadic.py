...
def handle_claim_territory(query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    t = min(unclaimed_t, key=lambda x: len(f_empty(x)))
    ...
    for t1 in my_t:
        if T(t1) <= 2:
            ...
            for t2 in T2:
                if T(t2) > 1:
                    to_move = T(t2) // 2
                    assert to_move >= 1 and T(t2) - to_move >= 1
                    return game.move_fortify(query, t2, t1, to_move)
