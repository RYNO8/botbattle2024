...
SPREE_THRESH, LEAVE_T, SCORE_D, SCORE_C, CARD_THRESH = 9, 3, 1, 0, 7
...
class BotState():
    ...
    def getReqT(self) -> int:
        ...
        return max(0, sum(T(x)*.85+1 for x in self.targets[1:]) - T(self.targets[0]))
    ...
    def getBestPath(self, my_t: list[int], border_t: list[int], total_troops: int):
        ...
        for _ in range(200):
            ...
            def updatePath():
                ...
                currScore = (cntDefeated if SCORE_D else numDefeated, cntContinents if SCORE_C else numContinents, -len(border), len(path))
                nonlocal bestScore
            ...
            while True:
                opp_x = [x for x in f_adj(path[-1]) if x not in seen and remT-T(x)*.85-1>=0]
                if not opp_x:
                    updatePath()
                    break
