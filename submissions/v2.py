...
def find_shortest_path_from_vertex_to_set(source: int, target_set: set[int]) -> list[int]:
    ...
    print(source, target_set, path)
    return path

def f_num_defenders(t):
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    return sum(x in my_territories for x in game.state.map.get_adjacent_to(t))
...
def handle_claim_territory(bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    selected_territory = sorted(unclaimed_territories, key=f_num_defenders, reverse=True)[0]
    return game.move_claim_territory(query, selected_territory)


def handle_place_initial_troop(bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    initial_troop = min(border_territories_id, key=f_curr_troops)
    return game.move_place_initial_troop(query, initial_troop)

def handle_redeem_cards(bot_state: BotState, query: QueryRedeemCards) -> MoveRedeemCards:
...
def handle_distribute_troops(bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    potential = lambda t: distributions[t] + f_curr_troops(t)
...
def handle_attack(bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    def attack_weakest(territories: list[int]) -> Optional[MoveAttack]:
        ...
        territories = sorted(territories, key=f_curr_troops)
        for candidate_target in territories:
            candidate_attackers = sorted(intersect(game.state.map.get_adjacent_to(candidate_target), my_territories), key=f_curr_troops, reverse=True)
    ...
    if len(game.state.recording) < 4000:
        ...
        # If we have no enemy, we will pick the player with the weakest territory bordering us, and make them our enemy.
            ...
            weakest_territory = min(bordering_territories, key=f_curr_troops)
        ...
        enemy_territories = intersect(bordering_territories, game.state.get_territories_owned_by(enemy))
    ...
    # In the late game, we will attack anyone adjacent to our strongest territories (hopefully our doomstack).
        ...
        strongest_territories = sorted(my_territories, key=f_curr_troops, reverse=True)
...
def handle_troops_after_attack(bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
...
def handle_defend(bot_state: BotState, query: QueryDefend) -> MoveDefend:
...
def handle_fortify(bot_state: BotState, query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    most_troops_territory = max(candidate_territories, key=f_curr_troops)
    ...
    def choose_move(query: QueryType) -> MoveType:
        match query:
            case QueryClaimTerritory() as q:
                return handle_claim_territory(bot_state, q)

            case QueryPlaceInitialTroop() as q:
                return handle_place_initial_troop(bot_state, q)

            case QueryRedeemCards() as q:
                return handle_redeem_cards(bot_state, q)

            case QueryDistributeTroops() as q:
                return handle_distribute_troops(bot_state, q)

            case QueryAttack() as q:
                return handle_attack(bot_state, q)

            case QueryTroopsAfterAttack() as q:
                return handle_troops_after_attack(bot_state, q)

            case QueryDefend() as q:
                return handle_defend(bot_state, q)

            case QueryFortify() as q:
                return handle_fortify(bot_state, q)
