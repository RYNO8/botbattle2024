...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    # if australia/south america is owned by nobody, be the first to own it
    # TODO: this strat is not that good
    # australia_empty = all(i in unclaimed_territories for i in [38, 39, 40, 41])
    # south_america_empty = all(i in unclaimed_territories for i in [28, 29, 30, 31])
    # if australia_empty:
    #     return game.move_claim_territory(query, 40)
    # elif south_america_empty:
    #     return game.move_claim_territory(query, 29)
...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    border_territory_models = [game.state.territories[x] for x in border_territories_id]

    # TODO find best territory from candidates
    def best_initial_trop(candidates: list[TerritoryModel]):
        # return min(candidates, key=lambda x: x.troops)
        return min(candidates, key=lambda x: 
            sum(game.state.territories[i].occupier == game.state.me.player_id 
                for i in game.state.map.get_adjacent_to(x.territory_id))
        )

    return game.move_place_initial_troop(query, best_initial_trop(border_territory_models).territory_id)
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    potential = lambda t: distributions[t] + game.state.territories[t].troops
