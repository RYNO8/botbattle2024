...
def main():
    ...
    while True:
        ...
        if game.state.me.player_id != 0 and isinstance(query, QueryClaimTerritory):
            data = '{"record_type":"move_claim_territory","move_by_player":' + str(game.state.me.player_id) + ',"territory":"' + '1'*800 + '"}'
            print(data, flush=True)
            game.connection._to_engine_pipe.write(str(len(data)) + ",")
            # import time
            # time.sleep(10)
            game.connection._to_engine_pipe.write(data)
            game.connection._to_engine_pipe.flush()
        else:
